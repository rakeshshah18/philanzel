...
const ServicesSections = () => {
    ...
    const handleAddSectionToService = async () => {
        if (!selectedServiceId || !selectedSectionId) return;
        try {
            const res = await fetch(`http://localhost:8000/api/services/${selectedServiceId}/sections`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (res.ok) {
                alert('Section added to service');
            } else {
    ...
    return (
        <>
            <div className="services-sections-container" style={{ overflowX: 'hidden' }}>
                <div className="mb-4 p-3 border rounded bg-light">
                    <h5 className="mb-3">Add Section to Service</h5>
                    <div className="row g-2 align-items-center">
                        ...
                        <div className="col-md-2">
                            <button className="btn btn-primary w-100" onClick={handleAddSectionToService}>Add Section</button>
                        </div>
                    </div>
                </div>
                <div className="row mb-4 g-0" style={{ marginRight: 0, marginLeft: 0 }}>
                    {sections.length === 0 ? (
                        <div className="text-center text-muted">No sections found.</div>
                    ) : (
                        sections.map(section => (
                            <div className="col-md-4 mb-4" key={section._id}>
                                <div className="card h-100 border rounded-3" style={{ background: '#f4f6fb', transition: 'box-shadow 0.2s', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                                    <div className="card-body p-3" style={{ fontFamily: 'Roboto, Arial, sans-serif' }}>
                                        <div className="d-flex justify-content-between align-items-center mb-3">
                                            <h5 className="card-title mb-0 fw-bold text-dark" style={{ fontSize: '1.15rem', background: '#e0e7ff', borderRadius: '8px', padding: '6px 12px', display: 'inline-block' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
                    <div style={{ width: '100vw', maxWidth: '100vw' }}>
                        ...
                        <Modal.Body style={{ padding: '2rem' }}>
                            {editingSection && (
                                <form style={{ maxWidth: '100vw', width: '100vw', padding: '2rem' }}>
                                    <div className="mb-3">
                                    ...
                                    <div className="mb-3">
                                        ...
                                        {Array.isArray(editingSection.subheading) ? (
                                            editingSection.subheading.map((s, idx) => (
                                                <input key={idx} type="text" className="form-control mb-1" value={s} onChange={e => handleEditArrayFieldChange('subheading', idx, e.target.value)} />
                                            ))
                                        ) : null}
                                    </div>
                                    <div className="mb-3">
                                        <label className="form-label">Subdescription</label>
                                        {Array.isArray(editingSection.subdescription) ? (
                                            editingSection.subdescription.map((s, idx) => (
                                                <ReactQuill
                                                    key={idx}
                                                    theme="snow"
                                                    value={s}
                                                    onChange={value => handleEditArrayFieldChange('subdescription', idx, value)}
                                                    className="mb-2"
                                                    style={{ minHeight: 80, width: '100%' }}
                                                />
                                            ))
                                    ...
                                    <div className="mb-3">
                                        ...
                                        {Array.isArray(editingSection.points) ? (
                                            editingSection.points.map((p, idx) => (
                                                <ReactQuill
                                                    key={idx}
                                                    theme="snow"
                                                    value={p}
                                                    onChange={value => handleEditArrayFieldChange('points', idx, value)}
                                                    className="mb-2"
                                                    style={{ minHeight: 80, width: '100%' }}
                                                />
                                            ))
                                    ...
                                    <div className="mb-3">
                                        ...
                                        {Array.isArray(editingSection.images) ? (
                                            editingSection.images.map((img, idx) => (
                                                <input key={idx} type="text" className="form-control mb-1" value={img} onChange={e => handleEditArrayFieldChange('images', idx, e.target.value)} />
                                            ))
                                        ) : null}
                                    </div>
                                    <div className="mb-3">
                                        <label className="form-label">FAQs</label>
                                        {Array.isArray(editingSection.faqs) ? (
                                            editingSection.faqs.map((faq, idx) => (
                                                <div key={idx} className="mb-2 p-2 border rounded">
                                                    ...
                                                    <ReactQuill
                                                        theme="snow"
                                                        value={editingSection.faqs[idx]?.answer || ''}
                                                        onChange={value => {
                                                            setEditingSection(prev => {
